defmodule Zigler.Zig do
  ...
  def makefor("[]c_int"),      do: "return beam.make_c_int_list(env, result) catch { return beam.raise_enomem(env); };"
  def makefor("[]c_long"),     do: "return beam.make_c_long_list(env, result) catch { return beam.raise_enomem(env); };"
  def makefor("[]i32"),        do: "return beam.make_i32_list(env, result) catch { return beam.raise_enomem(env); };"
  def makefor("[]i64"),        do: "return beam.make_i64_list(env, result) catch { return beam.raise_enomem(env); };"
  def makefor("[]f16"),        do: "return beam.make_f16_list(env, result) catch { return beam.raise_enomem(env); };"
  def makefor("[]f32"),        do: "return beam.make_f32_list(env, result) catch { return beam.raise_enomem(env); };"
  def makefor("[]f64"),        do: "return beam.make_f64_list(env, result) catch { return beam.raise_enomem(env); };"
